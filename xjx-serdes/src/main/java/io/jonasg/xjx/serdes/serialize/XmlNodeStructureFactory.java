...
public class XmlNodeStructureFactory {
    ...
    public static final List<Class<?>> BASIC_TYPES = List.of(String.class, Integer.class, Boolean.class, boolean.class, Long.class, long.class, BigDecimal.class, Double.class, double.class, char.class, Character.class, LocalDate.class, LocalDateTime.class, ZonedDateTime.class, byte[].class);
    ...
    private void handleLeafNode(InstanceField field, Section section, Tag tag, XmlNode node) {
        if (!tag.attribute().isEmpty()) {
            if (field.getValue() != null) {
                node.addNode(section.name()).addAttribute(tag.attribute(), field.getValue());
            }
        } else {
